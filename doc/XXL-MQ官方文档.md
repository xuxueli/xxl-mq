...
- 1、[迭代中]访问令牌（accessToken）：为提升系统安全性，配置中心和客户端进行安全性校验，双方AccessToken匹配才允许通讯；
- 2、底层long polling监控keys非法去重问题修复；
- 3、[迭代中]注册逻辑优化，批量注册，服务注册KV=N:1，但是消费者注册需要KV=M:N；
- 4、[迭代中] 性能测试；消息新增、成功率方面；
- 5、[迭代中] 独立MQ注册中心，与RPC解耦；注册支持数组 [key:value] 格式；
