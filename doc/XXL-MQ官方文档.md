...
- 5、通讯迁移至 xxl-rpc；
- 移除 spring 强依赖；
- 异步发送，提升消息发送性能；发送，Broker接口，消费全异步化；
- 串行消费优化，旧版本固定第一台消费，导致其压力过大；新版支持自定义shardingId从而实现串行消息的负载均衡，缓解单台压力；
- 广播消息优化，旧版本不支持消息持久化，新版本支持消息持久化，而且广播支持与串行结合实用，更加灵活；
- 并发消息、串行消息、广播消息模型，全部优化重构；逻辑统一；
- 支持自定义指定注册IP等信息，位置 "XxlMqBrokerImpl.initServer"；
- 兼容异步批量发送 + 同步发送；
- Topic自动发现：1min刷新一次，从最近5min的消息中发现新Topic；
- 运行报表：支持展示在线业务线、消息主题、消息记录等信息、可在线查看消息日期分布图，成功分布图等；
- 业务线管理：支持设置业务线，用于分组管理消息主题；
- 消息主题管理：支持在线管理消息主题，自动发现消息主题；并支持完善消息主题扩展信息，如业务线、负责人、告警邮箱等；
- 消息记录界面，交互优化重构；
- 消息在线清理功能；
- 消息发送改为多线程、异步方式；
- 超时强化，除了客户端支持超时控制外；服务端新增线程扫描，主动处理超时消息；消息超过 "生效时间 + 超时时间 + 1HOUT" 之后仍然未结束，将会主动标记为失败；
- 左侧菜单规范：运行报表（业务线，主题数，消息记录数；总消息成功率，日分布柱状图，总分布饼图） + 消息主题 + 消息记录 + 使用教程；
...
- 事务开关：支持设置消息事务开关，开启时事务保证消息精准消费一次；未开启时小概率存在重复消费，仅依靠注册中心分片检测避免重复，但性能略高；
- 告警功能：支持以Topic粒度监控消息，存在失败消息时主动推送告警邮件；
- 【ING】轨迹Log优化，新增、更新时记录核心数据；
...
- 客户端，Server端支持消息落磁盘；发送失败，存储失败时，写磁盘，避免消息丢失；
- 消息数据、Log使用text字段存储，为避免超长限制长度20000；后续考虑优化，尽量不限制数据长度、避免轨迹较多时Log超长问题；
- 消息告警功能增强，目前仅支持失败告警，考虑支持消息堆积突发流量告警、阻塞告警；
