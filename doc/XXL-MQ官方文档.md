...
> 注意：消息中心数据库，原生兼容支持 "MySQL、TIDB" 多存储介质；消息存储进行冷热设计，支持海量消息堆积（Mysql存储支持亿级；TiDB存储支持100亿级）；支持滚动式清理归档，开启后理论无容量上限；
...
* 建议指定版本号拉取镜像；
*/ 
docker pull xuxueli/xxl-mq-admin:{指定版本}
```
...
*/
docker run -d \
-e PARAMS="--spring.datasource.url=jdbc:mysql://127.0.0.1:3306/xxl_mq?useUnicode=true&characterEncoding=UTF-8&autoReconnect=true&serverTimezone=Asia/Shanghai" \
-p 8080:8080 \
-v /tmp:/data/applogs \
--name xxl-mq-admin \
xuxueli/xxl-mq-admin:{指定版本}
```
...
#### 4.3.2、海量数据堆积       
消息中心数据存储（Store），原生兼容支持 "MySQL、TIDB" 两种存储方式；消息存储进行冷热设计，支持海量消息堆积（Mysql存储支持亿级；TiDB存储支持100亿级）；支持滚动式清理归档，开启后理论无容量上限；
...
生产者启动时与Broker建立连接，生产者客户端负责将业务消息投递给Broker服务，Broker侧负责接受消息后进行分区路由以及存储，核心环节如下：
- 客户端生产消息：业务投递消息后，首先写入生产者内部客户端内存queue；
- 异步批量投递：客户端通过异步线程处理消息投递工作，批量读取queue中新消息数据，并基于消息中心OpenAPI进行批量消息投递；
- 消息分区路由：Broker接收投递消息后，数据首先进入内存queue；然后，Broker根据消息Topic设置的分区路由策略计算分区ID（固定1W分区）；
- 消息存储：消息分区路由计算完成后，更新消息分区ID信息，并最终写入消息Store中（XXL-MQ当前为存算分离架构，原生支持Mysql、TIDB）；  

至此，生产者消息生产及Broker存储落地完成。

#### 4.7.3、消费阶段

消费者启动时与Broker建立连接，消费者主动进行在线节点注册，通过动态分区分配和消费平衡机制，实现消费者责任分区范围分配，基于此消费者针对责任分区进行消息拉取和消费，核心环节为：
- 消费者注册：消费者启动后，主动进行在线节点注册，Broker可以实时感知消费者全量在线节点；
- 责任分区分配：Broker基于消费者在线节点信息，可针对每个消费者计算分配“责任分区范围”（系统固定总分区1W，计算所负责的分区区间）；
- 消费动态平衡：消费者新节点上线后，Broker支持集群间实时感知，并进行全局消费动态平衡，即重新进行责任分区分配；
- 消费者拉取：消费者完成“责任分区分配”后，消费者会通过Broker拉取所负责分区的消息数据；
- 消费者消费数据：消费者拉取到消息数据后会进行消费，并回调Broker更新消费结果。
...
- 2、【增强】海量数据堆积：消息数据存储在DB中，原生兼容支持 "MySQL、TIDB" 两种存储方式；
...
- 4、【增强】扩展支持多消息类型:
  ...
  - 延时消息：支持设置消息的延迟生效时间，到达设置的生效时间时该消息才会被消费。适用于延时消费场景，如订单超时取消、定时发送邮件等；
  - 失败重试消息：支持设置消息的失败重试次数，自定义重试间隔侧路，消息失败时会主动进行重试消费，直至重试次数耗尽或者消费成功。
- 5、【增强】海量消息：消息存储基于DB实现，支持Mysql、TiDB多存储介质；消息存储进行冷热设计，并滚动式清理归档，支持海量消息堆积。
...
### v1.6.0 Release Notes[2025-09-21]
- 1、【安全】登录安全升级，密码加密处理算法从Md5改为Sha256；
  ...
  ```
- 2、【优化】系统日志调整，支持启动时指定 -DLOG_HOME 参数自定义日志位置；同时优化日志格式提升易读性；
- 3、【优化】登录态持久化逻辑调整，简化代码逻辑；
- 4、【优化】异常页面处理逻辑优化，新增兜底落地页配置；
- 5、【优化】登录信息页面空值处理优化，避免空值影响ftl渲染；

### v1.7.0 Release Notes[2025-09-27]
- 1、【重构】UI框架重构升级，提升交互体验；
- 2、【优化】调整资源加载逻辑，移除不必要的拦截器逻辑，提升页面加载效率；
- 3、【优化】修改密码功能优化，要求新旧密码输入，提升安全性；
...
### v1.9.0 Release Notes[2025-12-28]
- 1、【升级】升级至 SpringBoot4；升级多项maven依赖至较新版本，如 spring、mybatis、xxl-sso 等；
- 2、【新增】新增支持通过“业务ID”进行消息建设和归档消息检索；
- 3、【优化】增加主题皮肤选项并优化界面交互；
...
### TODO
- 消费者线程优化：单消费者支持多线程并行消费；
- 消息管理界面优化：支持 生效时间、新增时间 多种排序方式；
