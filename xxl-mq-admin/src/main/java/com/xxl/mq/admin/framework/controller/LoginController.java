package com.xxl.mq.admin.framework.controller;

import com.xxl.mq.admin.framework.mapper.UserMapper;
import com.xxl.mq.admin.framework.model.entity.User;
...
public class LoginController {
	...
	@Resource
	private com.xxl.mq.admin.framework.service.UserService userService;
	@Resource
	...
	public Response<String> doLogin(HttpServletRequest request, HttpServletResponse response, String userName, String password, String ifRemember){
		...
		if (StringTool.isBlank(userName) || StringTool.isBlank(password)){
			return Response.ofFail( com.xxl.mq.admin.framework.util.I18nUtil.getString("login_param_empty") );
		}

		// valid user, empty、status、passowrd
		User user = userMapper.loadByUserName(userName);
		if (user == null) {
			return Response.ofFail( com.xxl.mq.admin.framework.util.I18nUtil.getString("login_param_unvalid") );
		}
		if (user.getStatus() != com.xxl.mq.admin.framework.constant.enums.UserStatuEnum.NORMAL.getValue()) {
			return Response.ofFail( com.xxl.mq.admin.framework.util.I18nUtil.getString("login_status_invalid") );
		}
